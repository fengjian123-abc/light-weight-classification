...
from models_Distillation_config.engine_dist import train_one_epoch, evaluate

from utils import NativeScalerWithGradNormCount as NativeScaler
import utils
import lcvtnet_models.model

from sampler import MultiScaleSamplerDDP
from fvcore.nn import FlopCountAnalysis

from models_Distillation_config.dist_args_parser import get_args_parser
from models_Distillation_config.model_dist_net import dist_model_config
from models_Distillation_config.losses_dist import Distillation_Loss
...
def main(args):
    ...
    for name,param in model.named_parameters():
        if "downsample_layers" not in name and "head" not in name and 'mlp' not in name: 
    ...
    # Dist Net Config
    dist_model_path='./models_Distillation_config/regnety_160-a5fe301d.pth'
    dist_model = dist_model_config(dist_model_path=dist_model_path,
                                   dist_model_type='regnety_160',
                                   n_classes=args.nb_classes,
                                   device=device)

    checkpoint = torch.load(dist_model_path, map_location='cpu')
    dist_model.load_state_dict(checkpoint['model'])
    # dist_model.load_state_dict(checkpoint)
    dist_model.to(device)
    dist_model.eval()

    criterion = Distillation_Loss(criterion,dist_model,'hard',0.5,1.0)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser('LCvTNet training and evaluation script', parents=[get_args_parser()])
